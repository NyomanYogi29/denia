...
- [x] **1.3 Setup Skema Database Drizzle (`src/core/db/schema.ts`)**
  - Tabel `users`, `bookings`, dan `force_events` diinisialisasi.
  - Menggunakan Drizzle v1 `defineRelations` dan SQLite indexing.
  - Client database `src/core/db/index.ts` dengan WAL mode & foreign keys.
...
- [x] **1.5.2 Standarisasi Custom Application Errors & Error Codes (`src/core/errors/`)**
  - `src/core/errors/codes.ts`: Definisi enum/konstanta `ErrorCode` lengkap.
  - `src/core/errors/app-error.ts`: Implementasi `AppError`, `UnauthorizedError`, `SlotConflictError`, `ValidationError`, `NotFoundError`.
  - `src/core/errors/resolver.ts`: Error resolver berbasis `switch-case` (`resolveError()`) untuk SQLite unique constraint mapping, pembuatan pesan WhatsApp Japri/DM, dan level log.
  - `src/core/errors/index.ts`: Barrel export.
- [x] **1.5.3 Standarisasi Kontrak Respon Sistem (Result Pattern) (`src/core/types/`)**
  - Buat `src/core/types/result.ts`: Generic type-safe `Result<T, E = AppError>` (`{ success: true, data: T } | { success: false, error: E }`).
  - Sediakan helper functions `ok<T>(data: T): Result<T>` dan `err<E>(error: E): Result<never, E>`.
  - Buat `src/core/types/index.ts` untuk barrel export.
- [x] **1.5.4 Template & Formatter Pesan WhatsApp (`src/core/templates/`)**
  - Buat `src/core/templates/batch-recap.ts`: Builder template rekap batch sukses untuk grup WA (header tanggal, daftar ruangan, jam SKS, peminjam).
  - Buat `src/core/templates/direct-message.ts`: Builder template penolakan/error khusus Japri/DM (penjelasan penyebab dan saran solutif).
  - Buat `src/core/templates/reaction.ts`: Standarisasi emoji reaksi status interaksi (`⏳` proses, `✅` sukses buffer, `❌` gagal).
  - Buat `src/core/templates/index.ts` untuk barrel export.

---

### 🔹 Fase 2: Kamus Domain & Validasi Inti (*Core Utilities*)
- [x] **2.1 Kamus Waktu Akademik SKS & Ruangan (`src/core/constants/slots.ts`, `src/core/constants/rooms.ts`)**
...
- [x] **2.3 Date & Room Parser (`src/core/utils/date.ts`, `src/core/utils/room-parser.ts`)**
